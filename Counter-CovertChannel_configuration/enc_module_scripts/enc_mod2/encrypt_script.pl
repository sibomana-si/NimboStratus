...
sub Encrypt
  {
	...
	copy($fileURL, $tmpEnc);
	`sisigcrypt -e -f $file -k $encKey -i $encIV`;
	unlink $file;
 	return $output;
  }
...
sub Main
  {
	...
	# "decrypted" directory(which triggers this script), is from the CSP and 
	# has just been decrypted, or is from the user and needs to be encrypted 
	# before being sent to the CSP.
